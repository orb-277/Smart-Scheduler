...
def lay_schedule_per_zone(start_date,zone_week_numbers,week_capsules,schedule_dict,total_weeks,filename:str):
    repeat_cycler = []
    z = 0
    ...
    for i in range(len(zone_week_numbers)):
        ...
        if check_week_capsule_health(week_capsules[repeat_cycler[r_c]]) == "low":
             ...
             lay_schedule_per_zone(start_date,week_number_generator(zone_week_numbers[i],total_weeks),generate_week_capsules_alternate(random.choice(strong_zones),filename),schedule_dict,total_weeks,filename)
             return 
        else:
...
def schedule_generator(start_date: datetime.date, duration_months:int,filename:str):
    ...
    lay_schedule_per_zone(start_date,east_numbers,east_capsules,schedule_dict,total_weeks,filename)
    lay_schedule_per_zone(start_date,west_numbers,west_capsules,schedule_dict,total_weeks,filename)
    lay_schedule_per_zone(start_date,south_numbers,south_capsules,schedule_dict,total_weeks,filename)
    lay_schedule_per_zone(start_date,north_numbers,north_capsules,schedule_dict,total_weeks,filename)
    lay_schedule_per_zone(start_date,central_numbers,central_capsules,schedule_dict,total_weeks,filename)
...
truck2_schedule = []
mini_done = []
print(group_into_area_spec(nope_helper,filename_ex))
truck_2_capsule = generate_week_capsules_from_dict(group_into_area_spec(nope_helper,filename_ex),filename_ex)

tc2 = []
for i in truck_2_capsule:
    for j in i:
        tc2.append(j)
print(tc2)
for i in zone_list:
    for j in tc2:        
        try:
            if i[1].lower().strip() == get_zone_from_area_name(j[2],filename_ex).lower().strip():
                truck2_schedule.append((j,i[0]))
                tc2.remove(j)
                
            else:
                truck2_schedule.append([])
        except:
            pass
print(truck2_schedule)
#'Date','Society 1','Society 2','Area','Zone',"Week Number",'Standard','Flat Count'
tc2_final_sched = []
tc2_final_sched.append({'Date':'Truck 2 seperator','Society 1':'Truck 2 seperator','Society 2':'Truck 2 seperator','Area':'Truck 2 seperator',
        'Zone':'Truck 2 seperator','Week Number':'Truck 2 seperator',
        'Standard':'Truck 2 seperator',
        'Flat Count':'Truck 2 seperator'})
for i in truck2_schedule:
    try:
        tc2_final_sched.append({'Date':f'Any day in week {i[1]}','Society 1':i[0][0],'Society 2':i[0][1],'Area':i[0][2],
        'Zone':get_zone_from_area_name(i[0][2],filename_ex),'Week Number':i[1],
        'Standard':(get_standard_from_name(i[0][0],filename_ex),get_standard_from_name(i[0][1],filename_ex)),
        'Flat Count':(get_flat_count_from_name(i[0][0],filename_ex),get_flat_count_from_name(i[0][1],filename_ex))})
    except:
        pass
dict_data+=tc2_final_sched
dict_data+=nope
#excel conversion 
